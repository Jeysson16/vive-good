-- Complete reset of all progress-related functions
-- This ensures all old versions with incorrect column references are removed

-- Drop all existing functions that might have old references
...
-- Recreate calculate_user_progress function with correct column references
...
-- Recreate upsert_user_progress function
...
COMMENT ON FUNCTION upsert_user_progress(UUID, VARCHAR(255), TEXT) IS 'Upserts user progress data by calculating metrics from user habits and logs';