-- Corregir el cálculo de progreso para que sea diario en lugar de semanal acumulado
-- Esto resuelve el problema donde el progreso muestra 100% cuando no debería

-- Drop the existing function
DROP FUNCTION IF EXISTS calculate_user_progress(UUID);

-- Recreate the function with correct daily progress calculation
...
BEGIN
    ...
    WITH habit_stats AS (
        SELECT 
            ...
            -- Check if it's a suggested habit (all active habits are suggested)
            1 as is_suggested,
    ...
    SELECT 
        ...
        COALESCE(COUNT(*)::INTEGER, 0) as suggested_habits,
        ...
        -- FIXED: Calculate progress based on TODAY's completion, not weekly
        CASE 
            WHEN COUNT(*) > 0 THEN 
...
-- Add comment
COMMENT ON FUNCTION calculate_user_progress(UUID) IS 'Calculates user progress metrics based on TODAY completion, not weekly accumulation';