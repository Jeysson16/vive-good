-- MIGRACIÓN FINAL: CORRECCIÓN DEFINITIVA DE TIPOS EN STORED PROCEDURES
-- Eliminar COMPLETAMENTE todas las versiones y recrear con tipos TEXT consistentes
-- Fecha: 2025-01-10
-- Propósito: Resolver PostgrestException por conflicto de tipos character varying vs text

-- =====================================================
-- PASO 1: ELIMINAR TODAS LAS VERSIONES EXISTENTES
-- =====================================================

-- Eliminar todas las posibles versiones de las funciones con diferentes firmas
DROP FUNCTION IF EXISTS get_monthly_progress_metrics(UUID, DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_monthly_progress_metrics(UUID, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_monthly_progress_metrics(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_monthly_progress_metrics() CASCADE;

DROP FUNCTION IF EXISTS get_category_progress_metrics(UUID, DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_category_progress_metrics(UUID, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_category_progress_metrics(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_category_progress_metrics() CASCADE;

DROP FUNCTION IF EXISTS get_weekly_trend_metrics(UUID, DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_weekly_trend_metrics(UUID, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_weekly_trend_metrics(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_weekly_trend_metrics() CASCADE;

DROP FUNCTION IF EXISTS get_temporal_analysis_metrics(UUID, DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_temporal_analysis_metrics(UUID, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_temporal_analysis_metrics(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_temporal_analysis_metrics() CASCADE;

-- =====================================================
-- PASO 2: RECREAR FUNCIONES CON TIPOS TEXT CONSISTENTES
-- =====================================================

-- 1. FUNCIÓN: get_monthly_progress_metrics
-- CRÍTICO: TODOS los tipos de retorno deben ser TEXT
...
BEGIN
    ...
    -- CRÍTICO: Retornar SOLO tipos TEXT para evitar PostgrestException
    RETURN QUERY
...
-- Permisos para get_monthly_progress_metrics
...
-- Permisos para get_category_progress_metrics
...
-- Permisos para get_weekly_trend_metrics
...
-- Permisos para get_temporal_analysis_metrics
...
'MIGRACIÓN 044: Obtiene métricas mensuales de progreso con tipos TEXT consistentes para evitar PostgrestException. Elimina conflictos character varying vs text.';

COMMENT ON FUNCTION get_category_progress_metrics(UUID, DATE, DATE) IS 
'MIGRACIÓN 044: Obtiene métricas de progreso por categoría con tipos TEXT consistentes para evitar PostgrestException. Elimina conflictos character varying vs text.';

COMMENT ON FUNCTION get_weekly_trend_metrics(UUID) IS 
'MIGRACIÓN 044: Obtiene métricas de tendencias semanales con tipos TEXT consistentes para evitar PostgrestException. Elimina conflictos character varying vs text.';

COMMENT ON FUNCTION get_temporal_analysis_metrics(UUID, DATE, DATE) IS 
'MIGRACIÓN 044: Obtiene métricas de análisis temporal con tipos TEXT consistentes para evitar PostgrestException. Elimina conflictos character varying vs text.';
...
DO $$
BEGIN
    RAISE NOTICE 'MIGRACIÓN 044 COMPLETADA: Todas las funciones de progreso han sido recreadas con tipos TEXT consistentes';
    RAISE NOTICE 'Funciones disponibles:';
    RAISE NOTICE '- get_monthly_progress_metrics(UUID, DATE, DATE)';
    RAISE NOTICE '- get_category_progress_metrics(UUID, DATE, DATE)';
    RAISE NOTICE '- get_weekly_trend_metrics(UUID)';
    RAISE NOTICE '- get_temporal_analysis_metrics(UUID, DATE, DATE)';
    RAISE NOTICE 'PROBLEMA RESUELTO: PostgrestException por conflicto de tipos character varying vs text';
END $$;