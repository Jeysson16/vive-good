-- Migración para limpiar datos y corregir relaciones
-- Fecha: 2024-12-19
...
-- 2. Limpiar datos huérfanos en user_roles ANTES de crear la nueva relación
-- Mostrar registros que se van a eliminar
SELECT 
    'Registros huérfanos en user_roles que se eliminarán:' as info,
    COUNT(*) as count
...
-- 3. Ahora crear la relación correcta entre user_roles.user_id y auth.users.id
ALTER TABLE user_roles 
ADD CONSTRAINT user_roles_user_id_auth_users_fkey 
FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

-- 4. Verificar que existe el rol 'user' por defecto
...
-- 5. Crear función para el trigger automático
...
-- 7. Crear el trigger en auth.users
CREATE TRIGGER on_auth_user_created
    AFTER INSERT ON auth.users
    FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- 8. Crear perfiles faltantes para usuarios existentes
INSERT INTO public.profiles (id, first_name, last_name, email)
SELECT 
    u.id,
...
-- 9. Asignar roles faltantes para usuarios existentes
INSERT INTO public.user_roles (user_id, role_id)
SELECT 
    u.id,
    r.id
...
SELECT 
    'Usuarios sin perfil después de la migración' as status,
    COUNT(*) as count
FROM auth.users u
LEFT JOIN public.profiles p ON u.id = p.id
WHERE p.id IS NULL;
