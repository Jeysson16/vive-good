-- Eliminar y recrear la función get_habit_statistics con filtros de fecha corregidos
DROP FUNCTION IF EXISTS get_habit_statistics(UUID, INTEGER, INTEGER);
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN ml.best_dow = 0 THEN 'Domingo'
            WHEN ml.best_dow = 1 THEN 'Lunes'
            WHEN ml.best_dow = 2 THEN 'Martes'
            WHEN ml.best_dow = 3 THEN 'Miércoles'
            WHEN ml.best_dow = 4 THEN 'Jueves'
            WHEN ml.best_dow = 5 THEN 'Viernes'
            WHEN ml.best_dow = 6 THEN 'Sábado'
            ELSE 'Lunes'
        END::TEXT,
        CASE 
