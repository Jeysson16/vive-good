...
UPDATE habits SET 
    ...
    END,
    icon_color = CASE 
        WHEN LOWER(name) LIKE '%agua%' OR LOWER(name) LIKE '%beber%' OR LOWER(name) LIKE '%hidrat%' THEN '#00BCD4'
        WHEN LOWER(name) LIKE '%fruta%' OR LOWER(name) LIKE '%manzana%' OR LOWER(name) LIKE '%apple%' OR LOWER(name) LIKE '%almuerzo%' OR LOWER(name) LIKE '%comida%' OR LOWER(name) LIKE '%comer%' OR LOWER(name) LIKE '%restaurant%' THEN '#4CAF50'
        WHEN LOWER(name) LIKE '%caminar%' OR LOWER(name) LIKE '%walk%' OR LOWER(name) LIKE '%paso%' OR LOWER(name) LIKE '%estiramiento%' OR LOWER(name) LIKE '%stretch%' OR LOWER(name) LIKE '%flexibilidad%' OR LOWER(name) LIKE '%ejercicio%' OR LOWER(name) LIKE '%gym%' OR LOWER(name) LIKE '%fitness%' OR LOWER(name) LIKE '%entrenar%' THEN '#2196F3'
        WHEN LOWER(name) LIKE '%dormir%' OR LOWER(name) LIKE '%sueño%' OR LOWER(name) LIKE '%sleep%' OR LOWER(name) LIKE '%descanso%' THEN '#9C27B0'
        WHEN LOWER(name) LIKE '%meditar%' OR LOWER(name) LIKE '%meditación%' OR LOWER(name) LIKE '%mindfulness%' OR LOWER(name) LIKE '%espiritual%' OR LOWER(name) LIKE '%orar%' OR LOWER(name) LIKE '%spiritual%' THEN '#673AB7'
        WHEN LOWER(name) LIKE '%trabajo%' OR LOWER(name) LIKE '%work%' OR LOWER(name) LIKE '%oficina%' OR LOWER(name) LIKE '%planificar%' OR LOWER(name) LIKE '%plan%' OR LOWER(name) LIKE '%organizar%' THEN '#795548'
        WHEN LOWER(name) LIKE '%dinero%' OR LOWER(name) LIKE '%finanzas%' OR LOWER(name) LIKE '%money%' OR LOWER(name) LIKE '%ahorro%' THEN '#4CAF50'
        WHEN LOWER(name) LIKE '%casa%' OR LOWER(name) LIKE '%hogar%' OR LOWER(name) LIKE '%home%' OR LOWER(name) LIKE '%limpieza%' THEN '#FF5722'
        WHEN LOWER(name) LIKE '%social%' OR LOWER(name) LIKE '%amigos%' OR LOWER(name) LIKE '%familia%' OR LOWER(name) LIKE '%people%' THEN '#E91E63'
        WHEN LOWER(name) LIKE '%creativ%' OR LOWER(name) LIKE '%arte%' OR LOWER(name) LIKE '%dibujar%' OR LOWER(name) LIKE '%pintar%' THEN '#9C27B0'
        WHEN LOWER(name) LIKE '%película%' OR LOWER(name) LIKE '%movie%' OR LOWER(name) LIKE '%entretenimiento%' THEN '#FF9800'
        WHEN LOWER(name) LIKE '%diario%' OR LOWER(name) LIKE '%escribir%' OR LOWER(name) LIKE '%journal%' OR LOWER(name) LIKE '%write%' OR LOWER(name) LIKE '%leer%' OR LOWER(name) LIKE '%lectura%' OR LOWER(name) LIKE '%libro%' OR LOWER(name) LIKE '%read%' OR LOWER(name) LIKE '%estudio%' OR LOWER(name) LIKE '%estudiar%' OR LOWER(name) LIKE '%study%' OR LOWER(name) LIKE '%aprender%' THEN '#FF9800'
        ELSE '#6366F1'
    END
