-- Corregir el stored procedure para mostrar hábitos sin depender de calendar_events
-- El problema es que usa INNER JOIN con calendar_events, pero debería mostrar hábitos
-- basándose en user_habits y su configuración de frecuencia
...
-- Comment
COMMENT ON FUNCTION get_dashboard_habits(UUID, DATE) IS 'Obtiene todos los hábitos activos del usuario para el dashboard, basándose en user_habits y su configuración de frecuencia, sin depender de calendar_events';