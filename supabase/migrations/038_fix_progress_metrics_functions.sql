-- Corregir la función get_monthly_progress_metrics para arreglar el error de casting
...
BEGIN
    ...
    WITH daily_completions AS (
        ...
        WHERE uh.user_id = p_user_id
        AND uhl.status = 'completed'
        AND uhl.completed_at::date >= CURRENT_DATE - INTERVAL '30 days'
    ...
    consecutive_days AS (
        SELECT 
            ...
            CURRENT_DATE - (ROW_NUMBER() OVER (ORDER BY completion_date DESC) - 1) as expected_date
        ...
        SELECT 
            ...
            CASE 
                WHEN COUNT(DISTINCT uh.id) > 0 THEN 
                    (COUNT(DISTINCT CASE WHEN uhl.status = 'completed' THEN uhl.user_habit_id END)::DECIMAL / COUNT(DISTINCT uh.id)) * 100
                ELSE 0 
    ...
    -- Número de categorías diferentes con hábitos activos
    ...
    -- Comparar completados esta semana vs semana anterior
    WITH weekly_stats AS (
        SELECT 
            CASE 
                WHEN uhl.completed_at::date >= CURRENT_DATE - INTERVAL '7 days' THEN 'current_week'
                WHEN uhl.completed_at::date >= CURRENT_DATE - INTERVAL '14 days' THEN 'previous_week'
                ELSE 'other'
            END as week_period,
    ...
    -- 9. NEEDS ATTENTION CATEGORY (categoría con peor rendimiento)
    WITH category_performance AS (
        SELECT 
            c.name as category_name,
            COUNT(DISTINCT uh.id) as total_habits,
    ...
    FROM category_performance
    ORDER BY completion_rate ASC, total_habits DESC
    LIMIT 1;
