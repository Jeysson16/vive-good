-- Eliminar función existente y recrear con optimizaciones
...
BEGIN
    ...
    monthly_completions AS (
        ...
        INNER JOIN user_habit_logs uhl ON uhl.user_habit_id = (
            SELECT uh2.id FROM user_habits uh2 WHERE uh2.habit_id = uch.habit_id AND uh2.user_id = p_user_id LIMIT 1
        )
        WHERE uhl.completed_at >= DATE(p_year || '-' || LPAD(p_month::TEXT, 2, '0') || '-01')
          AND uhl.completed_at < DATE(p_year || '-' || LPAD(p_month::TEXT, 2, '0') || '-01') + INTERVAL '1 month'
