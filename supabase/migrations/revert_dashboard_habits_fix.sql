-- REVERTIR CAMBIOS PROBLEMÁTICOS EN get_dashboard_habits
-- Problema: INNER JOIN con calendar_events causa duplicados y muestra hábitos completados incorrectamente
-- Solución: Restaurar versión original con LEFT JOIN y lógica correcta
...
BEGIN
  ...
  SELECT 
    ...
    -- Calendar event information (LEFT JOIN para evitar duplicados)
  ...
  -- LEFT JOIN con calendar_events (NO INNER JOIN) para evitar duplicados
  LEFT JOIN (
    ...
    WHERE ce.user_id = p_user_id 
      ...
      -- REMOVIDO: AND ce.is_completed = false (esto causaba el problema)
    ORDER BY ce.habit_id, ce.start_time ASC
  ) ce ON h.id = ce.habit_id
...
-- Comment
COMMENT ON FUNCTION get_dashboard_habits(UUID, DATE) IS 'Obtiene todos los hábitos del usuario para el dashboard con información completa de progreso y estadísticas, incluyendo iconos específicos de cada hábito. Versión corregida sin duplicados.';